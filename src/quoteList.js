export const quoteList = [
  {
    champion: "Aatrox",
    quote: [
      "Paz, é a maior das mentiras",
      "A dor é temporária, a vitória é eterna",
      "Eu nao sou seu inimigo, eu sou O inimigo",
    ],
  },
  {
    champion: "Ahri",
    quote: ["Devo fazer sua pulsação subir ou sumir?"],
  },
  {
    champion: "Akali",
    quote: ["Falou"],
  },
  {
    champion: "Alistar",
    quote: ["Quem você chamou de Chifrudo?"],
  },
  {
    champion: "Amumu",
    quote: ["Posso te dar um abraço?"],
  },
  {
    champion: "Annie",
    quote: ["Eu nunca brinco com fósforos"],
  },
  {
    champion: "Aurelion Sol",
    quote: [
      "Criatura enfadonha",
      "Parabéns. Você mereceu o meu desprezo.",
      "As estrelas choram por você. Ahaha, mentira. Eu nem me lembro do seu nome.",
      "O amor é uma força tão real quanto a gravidade, experimente de vez em quando",
    ],
  },
  {
    champion: "Braum",
    quote: [
      "O coração é o músculo mais forte",
      "Visão é a chave da vitória... pedra tambem",
    ],
  },
  {
    champion: "Caitlyn",
    quote: ["Eu cuido do caso", "Procure não ficar muito tempo parado"],
  },
  {
    champion: "Camille",
    quote: [
      "A palavra certa corta mais profundamente que uma faca",
      "Nana neném, eu vim pra te matar...",
      "Precisão é a diferença entre um açougueiro e um cirurgião",
    ],
  },
  {
    champion: "Corki",
    quote: [
      "Lima Oscar Lima",
      "Tem alguem na minha cola",
      "Barão mágico para balão vermelho responda",
      "Isso só mostra que você é um grande covarde",
    ],
  },
  {
    champion: "Draven",
    quote: [
      "Rapaz eu sou demais",
      "Bem vindos a League of Draven",
      "Vamos admirar a mim por um momento",
    ],
  },
  {
    champion: "Dr. Mundo",
    quote: ["Mundo achar você frutinha"],
  },
  {
    champion: "Ekko",
    quote: ["O lance não é quanto tempo você tem, mas como você usa"],
  },
  {
    champion: "Evelynn",
    quote: [
      "Uma vez um homem me pediu pra eu me vestir direito... Aí eu vesti a pele dele",
    ],
  },
  {
    champion: "Ezreal",
    quote: [
      "Quem precisa de mapas?",
      "Seu lugar é no museu",
      "Existem muitos grandes magos por ai, mas nenhum é tão gostoso",
    ],
  },
  {
    champion: "Fiddlesticks",
    quote: ["Eu não tenho cérebro, e logo você também não terá"],
  },
  {
    champion: "Fiora",
    quote: ["Lâmina afiada, mente afiada"],
  },
  {
    champion: "Fizz",
    quote: ["Vou num mergulho e volto no outro"],
  },
  {
    champion: "Gragas",
    quote: ["Se é você que vai pagar, eu tô dentro!"],
  },
  {
    champion: "Graves",
    quote: ["Calma, amigo", "Foi só um tirinho de leve"],
  },
  {
    champion: "Irelia",
    quote: [
      "Ah elegancia curva, a força quebra",
      "Eu não mato pessoas, só mato noxianos",
    ],
  },
  {
    champion: "Ivern",
    quote: ["Toda vida é preciosa! É sim! Até a sua"],
  },
  {
    champion: "Ivern Rei Doce",
    quote: ["Egalité, Fraternité e doce de leite!"],
  },
  {
    champion: "Jarvan",
    quote: ["Hojé é um belo dia para morrer"],
  },
  {
    champion: "Jax",
    quote: [
      "Quem vai encarar o campeão?",
      "Pode vir",
      "Imagina se eu tivesse uma arma de verdade",
    ],
  },
  {
    champion: "Jayce",
    quote: ["Eu luto por um futuro iluminado", "Nunca um passo em falso"],
  },
  {
    champion: "Jhin",
    quote: [
      "Num massacre floresço, como uma flor no amanhecer",
      "Sua vida não tinha valor antes de mim",
      "Bem na deixa",
      "Invejo o silêncio pois devo ser estrondoso",
    ],
  },
  {
    champion: "Jinx",
    quote: [
      "Digam olá para meus amigos de vários tamanhos",
      "Regras são feitas para serem quebradas. Igual prédios... Ou pessoas!",
      "Sou doida, tenho um atestado",
    ],
  },
  {
    champion: "Kai'Sa",
    quote: ["Levante-se, persista e vá pra cima"],
  },
  {
    champion: "Kalista",
    quote: ["Morte a todos os traidores"],
  },
  {
    champion: "Karma",
    quote: ["Seu espirito é algo que ninguem pode tirar de você"],
  },
  {
    champion: "Katarina",
    quote: [
      "A violência resolve tudo",
      "Vamos lá, aproveite a vida... enquanto pode!",
    ],
  },
  {
    champion: "Kayle",
    quote: ["FASCÍNORA!", "Os injustos queimaram"],
  },
  {
    champion: "Kayle Asas Etéreas",
    quote: ["Bandido bom é bandido morto"],
  },
  {
    champion: "Kindred",
    quote: [
      "O amanhã é uma esperança, nunca uma promessa",
      "Nunca um sem o outro",
    ],
  },
  {
    champion: "Lee Sin",
    quote: [
      "Domine a sí mesmo e dominará o inimigo",
      "Sua vontade, minhas mãos",
    ],
  },
  {
    champion: "Lulu",
    quote: ["Tem um gosto meio roxo"],
  },
  {
    champion: "Lux",
    quote: [
      "Se você quiser sorrir pode também iluminar",
      "Vamos iluminar isso aqui",
    ],
  },
  {
    champion: "Master Yi",
    quote: [
      "Olhe para o seu interior",
      "Dúvida é o maior inimigo",
      "Mente concentrada pode perfurar rochas",
      "Um verdadeiro mestre é um eterno aprendiz",
      "O segredo para a imortalidade?  Não morra!",
      "Em mim o Wuju perdura",
      "Raiva dá motivação sem propósito",
    ],
  },
  {
    champion: "Mordekaise",
    quote: ["A miséria ama companhia"],
  },
  {
    champion: "Morgana",
    quote: [
      "A justiça é uma mentira dos poderosos",
      "Nem todos os anjos são bons",
      "A verdadeira graça é bela em sua imperfeição, honesta em suas emoções e livre em sua fragilidade.",
    ],
  },
  {
    champion: "Nasus",
    quote: [
      "O ciclo da vida e da morte continua, nós viveremos, eles morrerão",
    ],
  },
  {
    champion: "Nocturne",
    quote: ["Escuridão", "Completamente sozinho"],
  },
  {
    champion: "Nami",
    quote: [
      "Ewww, bafo de Baleia",
      "Nas horas vagas eu sou uma encantadora de tubarões",
    ],
  },
  {
    champion: "Neeko",
    quote: [
      "Neeko não é tomatinho triste, neeko é tomatinho fortona!",
      "Dividir lembranças, cores e sentimentos é mágico! Magia conecta as pessoas!",
    ],
  },
  {
    champion: "Nunu",
    quote: ["Não irrite o Yeti. Não vai gostar de ver ele irritado."],
  },
  {
    champion: "Poppy",
    quote: ["Tive uma ideia! 3 rabinhos de cavalo"],
  },
  {
    champion: "Pyke",
    quote: ["O que vale mais? Um quilo de carne ou um galão de sangue?"],
  },
  {
    champion: "Quinn",
    quote: ["Olho neles!"],
  },
  {
    champion: "Rakan",
    quote: [
      "Nada do que eu falo deve ser levado a sério, por isso que é verdade.",
    ],
  },
  {
    champion: "Rammus",
    quote: ["Ok"],
  },
  {
    champion: "Renekton",
    quote: ["Enquanto eu viver, todos morrerão"],
  },
  {
    champion: "Riven",
    quote: ["A violencia só gera violencia"],
  },
  {
    champion: "Ryze",
    quote: ["Sabe oq cairia bem agora? Uma gelada"],
  },
  {
    champion: "Sejuani",
    quote: ["Viver com medo não é viver"],
  },
  {
    champion: "Sett",
    quote: [
      "A parada não é quantas vezes te derrubam, é quantas vezes você levanta",
      "Não tem pra ninguém.",
    ],
  },
  {
    champion: "Shaco",
    quote: ["Você é a piada aqui", "Que tal um truque de mágica?"],
  },
  {
    champion: "Shen",
    quote: ["Você já esta morto, só não percebeu ainda"],
  },
  {
    champion: "Singed",
    quote: ["Batido, não misturado"],
  },
  {
    champion: "Sona",
    quote: [""],
  },
  {
    champion: "Soraka",
    quote: [
      "Enquanto eu viver , ninguém há de sofrer",
      "Sim, aquilo era uma banana, ninguem espera por uma banana!",
      "Sim, isto é um chifre nascendo na minha testa",
    ],
  },
  {
    champion: "Tahm Kench",
    quote: ["Você é tão honesta quanto uma cortesã recentemente paga"],
  },
  {
    champion: "Talon",
    quote: ["Somente os tolos colocam a vida em risco pela honra"],
  },
  {
    champion: "Taric",
    quote: [
      "Estamos todos indo em direção ao mesmo fim inevitável. Não precisa ter pressa!",
      "Vê aquele brilho de esperança? Sou eu!",
    ],
  },
  {
    champion: "Teemo",
    quote: ["Tamanho não é documento"],
  },
  {
    champion: "Trundle",
    quote: ["Hora de trolar"],
  },
  {
    champion: "Thresh",
    quote: [
      "Eu? Louco? Ha! É bem capaz",
      "Que deleite de agonia vamo infligir",
      "Eu sou o monstro debaixo da cama",
      "Vira-latas imundos ficam na coleira",
    ],
  },
  {
    champion: "Twisted Fate",
    quote: [
      "A casa sempre vence",
      "A sorte sempre me sorri",
      "Encantado",
      "Tenho dois coringas no baralho, não preciso de mais um palhaço como você",
    ],
  },
  {
    champion: "Twitch",
    quote: [
      "É meeeeeu, eu que lambi",
      "O que não mata ainda não terminou",
      "Diretamente dos esgotos vem o extermínio",
      "SOU EEEUUUUUUUU!",
    ],
  },
  {
    champion: "Yasuo",
    quote: [
      "A morte é como o vento: está sempre ao seu lado",
      "Seria o unico proposito da folha o de cair?",
      "Certezas da vida: Honra, morte e ressaca. Ouro garantido",
    ],
  },
  {
    champion: "Yuumi",
    quote: ["E por isso que Yuumi rima com destruir"],
  },
  {
    champion: "Zed",
    quote: ["A lâmina que não se vê é a mais mortífera", "Ignorância é fatal"],
  },
  {
    champion: "Zyra",
    quote: ["Nossas estações estão trocadas, minha primavera, é o seu inferno"],
  },
];

export default {
  quoteList,
};
